...
//			calc_out_data(pc16, 0); // 한바뀌 읽었다
	...
	//			SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_LABEL_TEXT, "측 정 중");
		...
		else
		{
			...
			sprintf(temp,"end pos sig1 :%f - %f", ChData[2], ChData[3]);
			LogDataFunc(temp, 1);
...
//			calc_out_data(pc16, 0); // 한바뀌 읽었다
		...
		else
		{
			...
			sprintf(temp,"end pos sig2 :%f - %f", ChData[2], ChData[3]);
			LogDataFunc(temp, 1);
			St1ReadIng = 0;
		}
	}
...
//			calc_out_data(pc16, 0); // 한바뀌 읽었다
	...
	//			SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_LABEL_TEXT, "측 정 중");
		...
		else
		{
			...
			sprintf(temp,"end pos sig3 :%f - %f", ChData[2], ChData[3]);
			LogDataFunc(temp, 1);
			St1ReadIng = 0;
		}
	}
...
//					calc_out_data(pc76, 1); // 반복측정 MODE
		...
		else
		{
			...
			MeasStart(0, 4); // Start: OFF,  Sig: 1
			sprintf(temp,"end pos sig4 :%f - %f", ChData[2], ChData[3]);
			LogDataFunc(temp, 1);
			St1ReadIng = 0;
		}
	}
