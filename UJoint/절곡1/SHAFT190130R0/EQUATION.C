...
//	계산식에서 Display가 Yes로된 계산식 이름
			...
			Model[IndexYes].Comp = Pgm[i].Comp;
			...
			Model[IndexYes].NMTol = Pgm[i].NMTol;
			Model[IndexYes].PMTol = Pgm[i].PMTol;
			Model[IndexYes].NM2Tol = Pgm[i].NM2Tol;
			Model[IndexYes].PM2Tol = Pgm[i].PM2Tol;
...
/*	
	GetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_NMTOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
	for(j = 0; j < MAX_EQU; j++)
		Pgm[j].NMTol = dtemp[j];
	
	GetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_PMTOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
	for(j = 0; j < MAX_EQU; j++)
		Pgm[j].PMTol = dtemp[j];
*/	
	GetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_NM2TOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
	for(j = 0; j < MAX_EQU; j++)
		Pgm[j].NM2Tol = dtemp[j];

/*	
	GetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_PM2TOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
	for(j = 0; j < MAX_EQU; j++)
		Pgm[j].PM2Tol = dtemp[j];
...
*/	
	for( i = 0; i < MAX_EQU; i++ )
		dtemp[i] = Pgm[i].NM2Tol;
	SetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_NM2TOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
/*		
	for( i = 0; i < MAX_EQU; i++ )
		dtemp[i] = Pgm[i].PM2Tol;
	SetTableCellRangeVals (equsetup, EQUSETUP_EQUATION,
						  	MakeRect (1, PGM_PM2TOL_COL, MAX_EQU, 1),
						  	dtemp, VAL_COLUMN_MAJOR);
*/	
	...
	for( i = 0; i < MAX_EQU; i++ )
	SetTableCellRangeAttribute (equsetup, EQUSETUP_EQUATION, MakeRect(i + 1, PGM_NM2TOL_COL, 1, 1), ATTR_PRECISION, Pgm[i].Unit);
