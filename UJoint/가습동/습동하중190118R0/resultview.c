...
*/
   ...
   InsertTableColumns (dataview, DATAVIEW_DATA, 1,
					   dataObj.measPointNum + 5, VAL_CELL_NUMERIC);
   //Deleteing the old rows and colums in DATA table

   //Setting the attributes of colums in DATA table
   	for(i = 1; i <= dataObj.measPointNum + 5; i++)
   	{  
		...
		if(i > dataObj.measPointNum)
		{
			...
			{
				SetTableColumnAttribute (dataview, DATAVIEW_DATA, i+4, ATTR_COLUMN_WIDTH, 240);
				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, i+4, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_STRING);
			
			}	
				else  if(i==  dataObj.measPointNum + 2) 
			{
				SetTableColumnAttribute (dataview, DATAVIEW_DATA, 1, ATTR_COLUMN_WIDTH, 180);
				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, 1, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_STRING);
			}
			
			else  if(i==  dataObj.measPointNum + 3)  
			{
				SetTableColumnAttribute (dataview, DATAVIEW_DATA, 2, ATTR_COLUMN_WIDTH, 180);
				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, 2, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_STRING);
			}
			else  if(i==  dataObj.measPointNum + 4)  
			{
				SetTableColumnAttribute (dataview, DATAVIEW_DATA, 3, ATTR_COLUMN_WIDTH, 180);
				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, 3, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_STRING);
			}
			else  if(i==  dataObj.measPointNum + 5)  
			{
				SetTableColumnAttribute (dataview, DATAVIEW_DATA, 4, ATTR_COLUMN_WIDTH, 50);
				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, 4, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_STRING);
			}
		...
		{
			SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, i+4, rowNum, 1), ATTR_CELL_TYPE, VAL_CELL_NUMERIC);

				SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, i+4, rowNum, 1), ATTR_PRECISION, dataObj.Unit[i - 1]);
			
		}

		
   	}
	SetTableCellRangeAttribute (dataview, DATAVIEW_DATA, MakeRect(1, 1, rowNum, dataObj.measPointNum + 5), ATTR_TEXT_POINT_SIZE, 18);
   ...
   for(i = 1; i <= dataObj.measPointNum ; i++)
   {
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, i+4,
      ...
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, dataObj.measPointNum+5,
							   ATTR_USE_LABEL_TEXT, 1);
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, dataObj.measPointNum+5,
							   ATTR_LABEL_TEXT, 
							   dataObj.pointName[dataObj.measPointNum+1 ]); 
   
   

      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 1,
							   ATTR_USE_LABEL_TEXT, 1);
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 1,
							   ATTR_LABEL_TEXT, 
							   dataObj.pointName[dataObj.measPointNum]); 
   
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 2,
							   ATTR_USE_LABEL_TEXT, 1);
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 2,
							   ATTR_LABEL_TEXT, 
							   dataObj.pointName[dataObj.measPointNum+2]); 
   
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 3,
							   ATTR_USE_LABEL_TEXT, 1);
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 3,
							   ATTR_LABEL_TEXT, 
							   dataObj.pointName[dataObj.measPointNum+3]); 
   
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 4,
							   ATTR_USE_LABEL_TEXT, 1);
      SetTableColumnAttribute (dataview, DATAVIEW_DATA, 4,
							   ATTR_LABEL_TEXT, 
							   dataObj.pointName[dataObj.measPointNum+4]); 
   ...
   for(i = startPos; i < startPos + rowNum ; i++)
   {
	  ...
	  temp[3][i-startPos] = dataObj.GNG2[i-1];
	  temp[4][i-startPos] = dataObj.GNG3[i-1];
   }
/*	  
...
*/	  
	  SetTableCellRangeVals(dataview, DATAVIEW_DATA, 
	                   		MakeRect (1,  1, rowNum, 1),
	                   		temp[0],
	                   		VAL_ROW_MAJOR
	                   		); // time
	  SetTableCellRangeVals(dataview, DATAVIEW_DATA, 
	                   		MakeRect (1, dataObj.measPointNum + 5, rowNum, 1),
	                   		temp[1],
	                   		VAL_ROW_MAJOR
	                   		); // Operator
	  SetTableCellRangeVals(dataview, DATAVIEW_DATA, 
	                   		MakeRect (1, 2, rowNum, 1),
	                   		temp[2],
	                   		VAL_ROW_MAJOR
	                   		); // Operator
 	  
	  SetTableCellRangeVals(dataview, DATAVIEW_DATA, 
	                   		MakeRect (1, 3, rowNum, 1),
	                   		temp[3],
	                   		VAL_ROW_MAJOR
	                   		); // Operator
	  SetTableCellRangeVals(dataview, DATAVIEW_DATA, 
	                   		MakeRect (1, 4, rowNum, 1),
	                   		temp[4],
	                   		VAL_ROW_MAJOR
	                   		); // Operator
   ...
   for(i = 1 ; i <= dataObj.measPointNum; i++)
   {
  	...
  	for( j=0;j<measNum; j++)
  	{
  	 	for( i=1; i<= dataObj.measPointNum; i++)
  	 	{
  	    	...
  	    	{
				SetTableCellAttribute (dataview, DATAVIEW_DATA, MakePoint(4,j+1) , ATTR_TEXT_COLOR, VAL_RED);
   				SetTableCellAttribute (dataview, DATAVIEW_DATA, MakePoint(i+4,j+1) , ATTR_TEXT_COLOR, VAL_RED);
			}
			else
   				SetTableCellAttribute (dataview, DATAVIEW_DATA, MakePoint(i+4,j+1) , ATTR_TEXT_COLOR, VAL_BLACK);
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
		...
		case EVENT_COMMIT: 
			...
			if( FileOpenFlag == MANU_DATA )
			{
				Status = FileSelectPopup (DataFileBuff, "*.MAN", "*.OK;*.NG;*.ALL;*.LFT;*.RGT;*.MAN", "DATA LOAD",
										  VAL_LOAD_BUTTON, 0, 0, -1, 0, FileName1);
				FileOpenFlag = NO_DATA;
			}
			else // if( FileOpenFlag == AUTO_DATA )
			{
				Status = FileSelectPopup (DataFileBuff, "*.ALL", "*.OK;*.NG;*.ALL;*.LFT;*.RGT;*.MAN", "DATA LOAD",
										  VAL_LOAD_BUTTON, 0, 0, -1, 0, FileName1);
				FileOpenFlag = NO_DATA;
			}
...
//			ReadLine (hFile, dataObj.title, -1); // TITLE
			...
			ReadLine (hFile, DataFileBuff, -1);  // 측정 Point명
				for(i = 0 ; i < (dataObj.measPointNum) ; i++) // 측정 Point명 
				{
					...
					strncpy(ctemp, &DataFileBuff[(dataObj.measPointNum) * DATAFILE_PT_W + DATAFILE_NAME_W+ +DATAFILE_TIME_W+DATAFILE_OPT_W], DATAFILE_OPT_W);
					ctemp[DATAFILE_OPT_W] = 0; // set str-end
					FR_SpaceDelFunc( ctemp );
					strcpy(dataObj.pointName[dataObj.measPointNum+2], ctemp);
					
					strncpy(ctemp, &DataFileBuff[(dataObj.measPointNum) * DATAFILE_PT_W + DATAFILE_NAME_W+ +DATAFILE_TIME_W+DATAFILE_OPT_W+DATAFILE_OPT_W], DATAFILE_OPT_W);
					ctemp[DATAFILE_OPT_W] = 0; // set str-end
					FR_SpaceDelFunc( ctemp );
					strcpy(dataObj.pointName[dataObj.measPointNum+3], ctemp);
					
					strncpy(ctemp, &DataFileBuff[(dataObj.measPointNum) * DATAFILE_PT_W + DATAFILE_NAME_W+ +DATAFILE_TIME_W+DATAFILE_OPT_W+DATAFILE_OPT_W+DATAFILE_OPT_W], DATAFILE_NAME_W);
					ctemp[DATAFILE_NAME_W] = 0; // set str-end
					FR_SpaceDelFunc( ctemp );
			...
			{  
				...
				ctemp[DATAFILE_NAME_W] = 0; // set str-end
				FR_SpaceDelFunc( ctemp );
				strcpy(dataObj.GNG3[i], ctemp); // 작업자
...
*/    	   	
//			if((hFile = OpenFile (FileName1, VAL_READ_ONLY, VAL_OPEN_AS_IS, VAL_ASCII)) == -1)
			if((hFile = OpenFile (DataFileNameLFT, VAL_READ_ONLY, VAL_OPEN_AS_IS, VAL_ASCII)) == -1)
			{
			  sprintf(ctemp," FILE ERROR : %s",GetFmtIOErrorString(GetFmtIOError()) );
			  ConfirmPopup ("FILE OPEN ALARM !", ctemp);
			  return ;//FALSE;
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_COMMIT:
  		   		LaunchExecutable ("ResultView.exe");	
...
void	Save_CsvFile(void)
{
 ...
 if(startPos <= 1)
  ...
  if((hFile = OpenFile (FileName1, VAL_READ_WRITE, VAL_TRUNCATE,
						VAL_ASCII)) == -1)
  {
  	...
  	strcat(lineBuffer, "NAME"); 
  	// 계산식 이름 저장
	for( i=0; i< dataObj.measPointNum + 4;i++)
 	{   
  	...
  	for( j=startPos ; j<startPos+measNum ; j++)
  	{
     	lineBuffer[0]=0;
  	 	sprintf(lineBuffer,"%12d", j);
			...
			for( i=0; i< dataObj.measPointNum + 3;i++) 
  	 	{
  	 		if( i < dataObj.measPointNum )
  	 		{
				sprintf(fmt,", %%%d.%df",12,dataObj.Unit[i]);
...
//		sprintf(tempBuffer,"%%%d.%df",12,dataObj.Unit[i], dataObj.measVal[i][j-1]);
       			...
       			sprintf(tempBuffer,", %20s", dataObj.GNG1[j-1]);
