...
//				WriteLine (FileHandle, FileBuff,-1);
   ...
   sprintf(fmt, "%%%ds", DATAFILE_OPT_W);
	...
	strcat(FileBuff, DateStamp); // 12, 시간
...
//		sprintf(ctemp, "%9.3f",data[ManuRepeatMeasNo]);
		...
		{																//	측정완료
			if(GSt1Sig==2)
				calc_out_data(pc73, 0); // 반복 측정 신호
			else if(GSt1Sig==4)
				calc_out_data(pc74, 0); // 반복 측정 신호
//			else if(GSt1Sig==4)
//				calc_out_data(pc76, 0); // 반복 측정 신호
			...
			if(language==0)
			{
				SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_LABEL_TEXT, "LEFT 측정시작");
			}
...
//			MasterFlag = 0;
			...
			calc_out_data(pc69, 1); // 수동 측정요구 ST1
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		...
		case EVENT_LEFT_CLICK: 
			...
			calc_out_data(pc70, 1); // 수동 측정요구 ST1
...
*/
		...
		case EVENT_COMMIT:
		case EVENT_LOST_FOCUS:
			calc_out_data(pc70, 0); // 수동 측정요구 ST1
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "COMMIT", SET, MODELESS); // test
			break;
		}
	return 0;
...
//			MasterFlag = 0;
			...
			calc_out_data(pc86, 1); // 수동 측정요구 ST1
//			calc_out_data(pc15, 1); // 수동 측정요구 연속신호
			ManualOneTimeNo_1 = 0; // 반복 측정회수 확인용
			//	수동모드에서 설정된 연속측정 회수 구함
			GetCtrlVal(manualctrl, MANUALCTRL_CYCLENOSET_1, &ManualCycleNoSet_1);
...
//			MasterFlag = 0;
			...
			calc_out_data(pc87, 1); // 수동 측정요구 ST1
//			calc_out_data(pc15, 1); // 수동 측정요구 연속신호
			ManualOneTimeNo_1 = 0; // 반복 측정회수 확인용
			//	수동모드에서 설정된 연속측정 회수 구함
			GetCtrlVal(manualctrl, MANUALCTRL_CYCLENOSET_1, &ManualCycleNoSet_1);
...
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "CLICK", SET, MODELESS); // test
		...
		case EVENT_COMMIT:
		case EVENT_LOST_FOCUS:
			calc_out_data(pc87, 0); // 수동 측정요구 ST1
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "COMMIT", SET, MODELESS); // test
			break;
		}
	return 0;
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		{
		case EVENT_LEFT_CLICK: 
			calc_out_data(pc72, 1); // 수동 측정요구 ST1
...
*/
		...
		case EVENT_COMMIT:
		case EVENT_LOST_FOCUS:
			calc_out_data(pc72, 0); // 수동 측정요구 ST1
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "COMMIT", SET, MODELESS); // test
			break;
		}
	return 0;
...
//			MasterFlag = 0;
			...
			calc_out_data(pc71, 1); // 수동 측정요구 ST1
//			calc_out_data(pc15, 1); // 수동 측정요구 연속신호
...
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "CLICK", SET, MODELESS); // test
		...
		case EVENT_COMMIT:
		case EVENT_LOST_FOCUS:
			calc_out_data(pc71, 0); // 수동 측정요구 ST1
//			UserMessageBox(panel, 300, VAL_WHITE, VAL_RED, "COMMIT", SET, MODELESS); // test
			break;
		}
	return 0;
