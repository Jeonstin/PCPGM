		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		...
		case EVENT_LEFT_CLICK :
			...
			calc_out_data(pc8, 1); // ST1 ZsetReq
/*
			IdelStateTCP = 1; 		
			LogDataFunc("영점측정요구 ON", 0);
			ZeroSetCtrlSt1Dimmed(1);
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		{
		case EVENT_LEFT_CLICK :
			calc_out_data(pc61, 1); // ST1 ZsetReq
			break;
		case EVENT_COMMIT:
			calc_out_data(pc61, 0); // ST1 ZsetReq
			break;
		}
	return 0;
}
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
 		...
 		if( (ActivePanel == zerosetup) && AmpRunFlag )//  && !St1CalcIng ) // 수동측정 화면이고 측정중이 아니면
		{
			...
			SetTableCellRangeVals(zerosetup, ZEROSETUP_DATA_JNL, MakeRect(1, SENSOR_NAME, ProbeIndex, 1),
