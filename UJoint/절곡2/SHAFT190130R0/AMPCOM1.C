...
double		RepeatChValBase[MAX_ACQ_CH_PER_CYCLE][MAX_CH];
...
/*
			...
			if(!AmpPortOpenErr && (AmpModelID!=2) )
				RestAmp();
...
//		OrbitStatus = OrbitRead1(0,i+1,&rd);
			OrbitStatus = OrbitRead1(0,i+1,&rd);
//			real_rd = rd*5.0/16384.0;	 
			real_rd = (double)rd *ProbeV[i]/16384.0-10.0;
//			real_rd = (double)rd;///16384.0;
			
		ChData[i]= (real_rd-5.0)*20.0;//*1000;
...
//GSt1Sig	
		...
		if((GSt1Sig ) &&( St1MeasIng ||  (ActivePanel == manualctrl && RealTimeFlag)) )
		{

///////////////////			
			if( RepeatChIndex==0)
			{
//				Enc_Init = ChData[ProbeIndex-2];
...
/*
				...
				if((GSt1Sig==1)||(GSt1Sig==3))
				{
					ChData[2] = -40.0+(double)RepeatChIndex*0.1;
					ChData[3] = -40.0+(double)RepeatChIndex*0.1;
				}
...
*/				
			...
			{
				SetCtrlVal(mainpanel, MAINPANEL_LED_4,TRUE);
				SetCtrlVal(manualctrl, MANUALCTRL_LED_4,TRUE);
				RepeatChIndex++;
			}
