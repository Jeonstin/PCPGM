...
//	ChData[0] = (gData[0]-5.0)*20.0 ;
	

	Mean(gData, 100, &avg);
	ChData[0] = (avg-5.0)*20.0 ;
...
//	ChData[1] = (gData2[0]-5.0)*20.0 ;
	
	Mean(gData2, 100, &avg);
	ChData[1] = (avg-5.0)*20.0 ;
		...
		if(control==INTPANEL_CLOSE)
			switch (event)
				...
				case EVENT_COMMIT:
					...
					GetCtrlVal(interdata, INTPANEL_DATA_WEIGHT, &Data_Weight);
...
void	TurnDataGraphPlot1(int no,int	Index, int panel) // 
{
	...
	for( i = 0; i < Index; i++ )
	{
		...
		if((no==1))
			dtempX[i] = RepeatEncVal[i]+Shaft_Low-Shaft2_Low; // ENC
		if((no==3))
			dtempX[i] = RepeatEncVal[i]+Tube_Low-Tube2_Low; // ENC
...
//	SetAxisScalingMode (panel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 360);
		...
		if(no==0 || no == 2)
		{
			...
			SetAxisScalingMode (manualctrl, ctrlId2, VAL_XAXIS , VAL_MANUAL, RepeatEncVal[0], RepeatEncVal[Index-1]);
		...
		if(no==0 || no == 2)
		{
			...
			PlotXY(mainpanel, ctrlId, dtempX, L_GraphData.NTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_WHITE);
			PlotXY(mainpanel, ctrlId, dtempX, L_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			PlotXY(mainpanel, ctrlId, dtempX, Lm_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			...
			PlotXY(manualctrl, ctrlId2, dtempX, L_GraphData2.NTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_WHITE);
			PlotXY(manualctrl, ctrlId2, dtempX, L_GraphData2.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			
			PlotXY(manualctrl, ctrlId2, dtempX, Lm_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
...
//	SetAxisScalingMode (panel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 360);
		...
		if(no==0 || no == 2)
		{
			...
			PlotXY(manualctrl, ctrlId2, dtempX, L_GraphData2.NTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GRAY);
			PlotXY(manualctrl, ctrlId2, dtempX, L_GraphData2.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
...
void	CalculateSignal(int no)// low data에서 필요한 만큼만 뽑아서 RepeatChVal에 넣는다.
{
	...
	else if(no == 1 && GSt1Sig==2)
	{
		LowData = Shaft2_Low;
		HighData = Shaft2_High;
		
	}
	else if(no == 1 && GSt1Sig==3)
	{
		LowData = Shaft_Low;
		HighData = Shaft_High;
		
	}
	else if(no == 1 && GSt1Sig==4)
	{
		LowData = Shaft2_Low;
		HighData = Shaft2_High;
		
	}
	else if(no == 2 && GSt1Sig==1)
	{
		LowData = Tube_Low;
		HighData = Tube_High;
		
	}
	else if(no == 2 && GSt1Sig==2)
	{
		LowData = Tube2_Low;
		HighData = Tube2_High;
		
	}
	else if(no == 2 && GSt1Sig==3)
	{
		LowData = Tube_Low;
		HighData = Tube_High;
		
	}
	else if(no == 2 && GSt1Sig==4)
	{
		LowData = Tube2_Low;
		HighData = Tube2_High;
	...
	for( i = 0; i < RepeatChIndexBase; i++ ) // 
	{
		...
		if((data>= LowData) &&(data<= HighData) )
		{
			...
			if((RepeatChValBase[i][2]>= HighData)&&  (GSt1Sig==1) && (no == 1) )
				break;
			if((RepeatChValBase[i][3]>= HighData)&&  (GSt1Sig==1) && (no == 2) )
				break;

			if((RepeatChValBase[i][2]<= LowData)&&  (GSt1Sig==2) && (no == 1) )
				break;
			if((RepeatChValBase[i][3]<= LowData)&&  (GSt1Sig==2) && (no == 2) )
				break;

			
			if((RepeatChValBase[i][2]>= HighData)&&  (GSt1Sig==3) && (no == 1) )
				break;
			if((RepeatChValBase[i][3]>= HighData)&&  (GSt1Sig==3) && (no == 2) )
				break;

			if((RepeatChValBase[i][2]<= LowData)&&  (GSt1Sig==4) && (no == 1) )
				break;
			if((RepeatChValBase[i][3]<= LowData)&&  (GSt1Sig==4) && (no == 2) )
				break;
...
//	SetCtrlVal( mainpanel, MAINPANEL_LNG, McSPC[0].NG+McSPC[1].NG+McSPC[2].NG );
	SetCtrlVal( mainpanel, MAINPANEL_LG_3, Ok_cnt);
...
////shaft
		...
		if(Mc[McID].Mode[0].FinalResult[i+1][Mc[McID].Mode[0].EdDataNum - 1] > Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1])
			Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1] = Mc[McID].Mode[0].FinalResult[i+1][Mc[McID].Mode[0].EdDataNum - 1];
		
		sprintf(DataStamp,"%4.2f,", Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1]);
		strcat(TCPSendBuff, DataStamp);
		
		if((GFlag[i]==0)|| (GFlag[i+1]==0)) 
			...
			case	_C_A:  // Left
					...
					for( RepeatChIndex = 0; RepeatChIndex < 1000; RepeatChIndex++ )
					{
						for(i = 0; i < MAX_CH; i++)
						{
							...
							// (0 ~ RAND_MAX == 32767) <<-- rand()
							if(i==2 )
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/10.0-30);// shaft 각도
							else if(i==3 )
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/10.0-30);// tube 각도
							else if(i==0 )  //shaft 값
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/100 *1.5);
							else if(i==1 )  //shaft 값
