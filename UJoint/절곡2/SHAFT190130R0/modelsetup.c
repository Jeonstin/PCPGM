		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch(control)
	{
		...
		//		cell.x<=MODEL_PM2TOL_COL )
			if( cell.x>=MODEL_NTOL_COL  &&				// 설정가능한 Cell 이면
				cell.x<=MODEL_PTOL_COL )
			{
				...
				else if(cell.x==MODEL_NMTOL_COL)
					callKEYPAD(panel, control, event, TABLE, &Model[cell.y-1].NMTol, 0);
					
				else if(cell.x==MODEL_PMTOL_COL)
					callKEYPAD(panel, control, event, TABLE, &Model[cell.y-1].PMTol, 0);
				else if(cell.x==MODEL_NM2TOL_COL)
					callKEYPAD(panel, control, event, TABLE, &Model[cell.y-1].NM2Tol, 0);
					
				else if(cell.x==MODEL_PM2TOL_COL)
					callKEYPAD(panel, control, event, TABLE, &Model[cell.y-1].PM2Tol, 0);
	...
	for( i = col; i < col + width; i++ )
	{
		...
		for( j = row; j < row + height; j++ )
		{
			...
			switch( cell.x )
			{
				...
				case	PGM_NMTOL_COL:
					GetTableCellVal (equsetup, EQUSETUP_EQUATION, cell, &Pgm[cell.y - 1].NMTol);
		  			break;
				case	PGM_PMTOL_COL:
					GetTableCellVal (equsetup, EQUSETUP_EQUATION, cell, &Pgm[cell.y - 1].PMTol);
		  			break;

				case	PGM_NM2TOL_COL:
					GetTableCellVal (equsetup, EQUSETUP_EQUATION, cell, &Pgm[cell.y - 1].NM2Tol);
		  			break;
				case	PGM_PM2TOL_COL:
					GetTableCellVal (equsetup, EQUSETUP_EQUATION, cell, &Pgm[cell.y - 1].PM2Tol);
		  			break;
...
/////////////// 2진형식으로 제품종류를 전송
	...
	if( i & 0x01 )
		calc_out_data(pc65, 1); // 제품종류 2
	else
		calc_out_data(pc65, 0); // 제품종류 2

	if( i & 0x02 )
		calc_out_data(pc66, 1); // 제품종류 4
	else
		calc_out_data(pc66, 0); // 제품종류 4

	if( i & 0x04 )
		calc_out_data(pc67, 1); // 제품종류 8
	else
		calc_out_data(pc67, 0); // 제품종류 8

	if( i & 0x08 )
		calc_out_data(pc68, 1); // 제품종류 16
	else
		calc_out_data(pc68, 0); // 제품종류 16
...
/*	
	...
	///////////////////	
	GetTableCellRangeVals(modelsetup, MODELSETUP_DATA,
							MakeRect(1, MODEL_NMTOL_COL, MeasPoints, 1),
							dtemp, VAL_COLUMN_MAJOR);
	for( i = 0; i < MeasPoints; i++ )
		Model[i].NMTol = dtemp[i];
	///////////////////	
	GetTableCellRangeVals(modelsetup, MODELSETUP_DATA,
							MakeRect(1, MODEL_PMTOL_COL, MeasPoints, 1),
							dtemp, VAL_COLUMN_MAJOR);
	for( i = 0; i < MeasPoints; i++ )
		Model[i].PMTol = dtemp[i];

	///////////////////	
	GetTableCellRangeVals(modelsetup, MODELSETUP_DATA,
							MakeRect(1, MODEL_NM2TOL_COL, MeasPoints, 1),
							dtemp, VAL_COLUMN_MAJOR);
	for( i = 0; i < MeasPoints; i++ )
		Model[i].NM2Tol = dtemp[i];

	
	///////////////////	
	GetTableCellRangeVals(modelsetup, MODELSETUP_DATA,
							MakeRect(1, MODEL_PM2TOL_COL, MeasPoints, 1),
							dtemp, VAL_COLUMN_MAJOR);
	for( i = 0; i < MeasPoints; i++ )
		Model[i].PM2Tol = dtemp[i];
...
*/	
	...
	for( i = 0; i < MeasPoints; i++ )
	{
		...
		Pgm[Model[i].DispYesEquNo].NMTol = Model[i].NMTol;
		Pgm[Model[i].DispYesEquNo].PMTol = Model[i].PMTol;
		Pgm[Model[i].DispYesEquNo].NM2Tol = Model[i].NM2Tol;
		Pgm[Model[i].DispYesEquNo].PM2Tol = Model[i].PM2Tol;
...
*/
	...
	for( i = 0; i < MeasPoints; i++ )
	{
			SetTableCellRangeAttribute (modelsetup, MODELSETUP_DATA, MakeRect(i + 1, MODEL_NTOL_COL, 1, 2), ATTR_PRECISION, Model[i].Unit);
	}
