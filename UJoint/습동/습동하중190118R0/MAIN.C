...
static TaskHandle	gTaskHandle2=0;
static float64		*gData=NULL;
static float64		*gData2=NULL;
static uInt32		gNumChannels, gNumChannels2;
int	CNT, CNT2;
...
int32 CVICALLBACK DoneCallback2(TaskHandle taskHandle, int32 status, void *callbackData);
 
/////////
 
int		ErrDispCurNum, CalcTestMode;
int		AmpPortNo, PlcPortNo, MarkPortNo, AUX2PortNo;
//GRAPHDATA	GraphData, GraphData2;
char		DataFileNameOk[256], DataFileNameNg[256], DataFileNameAll[256], DataFileNameLFT[256], DataFileNameRGT[256];//, DataFile[256];
...
//	Project Directory
	...
	if( CmtNewThreadPool( 3, &poolHandle) )
	{
...
//		GetCtrlVal(panel,PANEL_MINVAL,&min);	 //-10.0
//		GetCtrlVal(panel,PANEL_MAXVAL,&max);	 //10.0
//		GetCtrlVal(panel,PANEL_RATE,&rate);		 //10000.0
//		GetCtrlVal(panel,PANEL_SAMPSPERCHAN,&sampsPerChan); //100
		
//		SetCtrlAttribute(panel,PANEL_STRIPCHART,ATTR_XAXIS_GAIN,1.0/rate);
//		SetCtrlAttribute(panel,PANEL_STRIPCHART_2,ATTR_XAXIS_GAIN,1.0/rate);
//		log = (int)log10(rate);
...
//		SetCtrlAttribute(panel,PANEL_STRIPCHART_2,ATTR_XPRECISION,log);
//		SetCtrlAttribute(panel,PANEL_STRIPCHART_2,ATTR_POINTS_PER_SCREEN,sampsPerChan);
		...
		DAQmxErrChk (DAQmxCreateAIVoltageChan(gTaskHandle,"Dev1/ai0","",DAQmx_Val_Cfg_Default,min,max,DAQmx_Val_Volts,NULL));
		...
		if( (gData2=(float64*)malloc(sampsPerChan*gNumChannels*sizeof(float64)))==NULL ) {
			MessagePopup("Error","Not enough memory");
			goto Error;
		}
  ...
  //////
		...
		DAQmxErrChk (DAQmxCreateAIVoltageChan(gTaskHandle2,"Dev1/ai1","",DAQmx_Val_Cfg_Default,min,max,DAQmx_Val_Volts,NULL));
		DAQmxErrChk (DAQmxCfgSampClkTiming(gTaskHandle2,"",rate,DAQmx_Val_Rising,DAQmx_Val_ContSamps,sampsPerChan));
		DAQmxErrChk (DAQmxGetTaskAttribute(gTaskHandle2,DAQmx_Task_NumChans,&gNumChannels2));



		DAQmxErrChk (DAQmxRegisterEveryNSamplesEvent(gTaskHandle2,DAQmx_Val_Acquired_Into_Buffer,sampsPerChan,0,EveryNCallback2,NULL));
		DAQmxErrChk (DAQmxRegisterDoneEvent(gTaskHandle2,0,DoneCallback2,NULL));
...
//		SetCtrlAttribute(panel,PANEL_STRIPCHART_2,ATTR_NUM_TRACES,gNumChannels2);
...
int32 CVICALLBACK EveryNCallback2(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void *callbackData)
{
	...
	DAQmxErrChk (DAQmxReadAnalogF64(taskHandle,nSamples,10.0,DAQmx_Val_GroupByScanNumber,gData2,nSamples*gNumChannels2,&numRead,NULL));
//	ChData[1] = gData2[0] ;
	Mean(gData2, 200, &avg);
	ChData[1] = (avg-5.0)*10.0 ;
...
// Project Directory로 환원
	...
	for( i = 0; i < 9; i++ )
		...
		// 	케이블 종류 체크  
			...
			sprintf(lineBuff,"%d   %d   %d   %d   %d   %d",AmpCableID, Comm_P[0],Comm_P[1],Comm_P[2],Comm_P[3], AmpModelID);
...
// Project Directory로 환원
	...
	for( i = 0; i < 9; i++ )
	{
		...
		if( !i )
			...
			Scan(lineBuff,"%s>%i %i %i %i %i %i", &AmpCableID, &AmpPortNo, &PlcPortNo, &MarkPortNo, &AUX2PortNo, &AmpModelID);
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		...
		case EVENT_COMMIT:
			...
			SetCtrlAttribute (manualctrl, MANUALCTRL_M_CHVALUE, ATTR_NUM_VISIBLE_ROWS,20 );
...
//						SetCtrlAttribute (manualctrl, MANUALCTRL_M_VIEW_2, ATTR_TOP, 1003);
//						SetCtrlAttribute (manualctrl, MANUALCTRL_MANUALTEST_M, ATTR_TOP, 1183);
//						SetCtrlAttribute (manualctrl, MANUALCTRL_MANUALTEST_M_2, ATTR_TOP, 1415);
//						SetCtrlAttribute (manualctrl, MANUALCTRL_M_GNG_3, ATTR_TOP, 1299);
//						SetCtrlAttribute (manualctrl, MANUALCTRL_M_GNG_4, ATTR_TOP, 1531);
					     
						SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_VISIBLE, TRUE);
...
//					GetCtrlVal(modelsetup, MODELSETUP_MODEL_CHAR, Model_Char);
...
//					SetCtrlVal(mainpanel, MAINPANEL_WORK_MODEL_CHAR, Model_Char);
					...
					GetCtrlVal(modelsetup, MODELSETUP_MODEL_CHAR_6, Model_Char[5]); //LINE
					GetCtrlVal(modelsetup, MODELSETUP_MODEL_CHAR_7, Model_Char[6]); //LINE
 ...
 /*
					if( ByPassFlag3)
					{
						SetCtrlVal( mainpanel, MAINPANEL_GNG_2, ON);				//	주화면에 합격표시
						SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_2, ON);				//	주화면에 합격표시
						SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_3, ON);				//	주화면에 합격표시
						SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_4, ON);				//	주화면에 합격표시
					...
					else
					{
						if(Zero_Master_Flag)
							...
							SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_3, OFF);				//	주화면에 합격표시

						if(Check_Master_Flag)
							SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_4, ON);				//	주화면에 합격표시
						else
							SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_4, OFF);				//	주화면에 합격표시
							
//						if(Check_Master_Flag && Zero_Master_Flag)
						if(Check_Master_Flag )
						{
						...
						{
							SetCtrlVal( mainpanel, MAINPANEL_GNG_2, OFF);				//	주화면에 합격표시
							SetCtrlVal( manualctrl, MANUALCTRL_M_GNG_2, OFF);				//	주화면에 합격표시
...
//					GetCtrlVal(optpanel, OPT_SET_SETCONTNG1, &ContNgChkNum);
					GetCtrlVal(optpanel, OPT_SET_BYPASS, &ByPassFlag);
					GetCtrlVal(optpanel, OPT_SET_BYPASS2, &ByPassFlag2);
/////수정 1219
//					GetCtrlVal(optpanel, OPT_SET_BYPASS3, &ByPassFlag3);
					GetCtrlVal(optpanel, OPT_SET_BYPASS4, &ByPassFlag4);//NICK BYPASSS
//					GetCtrlVal(optpanel, OPT_SET_BYPASS5, &ByPassFlag5);//NICK BYPASSS
...
*/
					...
					GetCtrlVal(interdata, INTPANEL_LOWLOG, &LowLogFlag);
...
/*
					if(i)
					{
						...
						SetCtrlAttribute (mainpanel, MAINPANEL_GRAPH1, ATTR_WIDTH, 444);
...
///////////////////
					...
					SetCtrlVal(mainpanel, MAINPANEL_ZERO_CNT, AutoZeroChkNum);
					...
					GetCtrlVal(interdata, INTPANEL_BYPASS3, &ByPassFlag3);
/*
					if( ByPassFlag3)
					{
						SetCtrlVal( mainpanel, MAINPANEL_GNG_2, ON);				//	주화면에 합격표시
...
*/
					GetCtrlVal(interdata, INTPANEL_BYPASS5, &ByPassFlag5);//NICK BYPASSS
					GetCtrlVal(interdata, INTPANEL_BYPASS6, &ByPassFlag4);//NICK BYPASSS
...
///////////
				...
				if( AutoZeroHourUseFlag && !ByPassFlag && !d_out[pc40] && d_in[plc4] ) // 자동 영점조정, 자동 기동중
				{
					GetSystemTime(&SystemH, &SystemM, &SystemS); // 현재의 시간 Data를 구함
...
void	TurnDataGraphPlot1(int no,int	Index, int panel) // 
{
 	double	dtempX[MAX_ACQ_CH_PER_CYCLE], dtempY[MAX_ACQ_CH_PER_CYCLE], dtempY2[MAX_ACQ_CH_PER_CYCLE], GngX[100];
 	double	ptol[MAX_ACQ_CH_PER_CYCLE], ntol[MAX_ACQ_CH_PER_CYCLE];
	int		i, j, ctrlId, ctrlId2;
	double	ResultData[MAXDATANUM];
	
	if( no == 0 || no == 1 ) //sig  번호
	{
		ctrlId = MAINPANEL_GRAPH3;
		ctrlId2 = MANUALCTRL_GRAPH3;
	}
	else if( no == 2 || no == 3 )
	{
		ctrlId = MAINPANEL_GRAPH5;
		ctrlId2 = MANUALCTRL_GRAPH5;
	}
	...
	{
		RepeatEncVal[i] = RepeatChVal_Sig3[i][2];//360.0*i/Index;
	}
...
//		else if(no==1)
//			dtempX[i] = RepeatEncVal[i]+Shaft_Low; // ENC
...
/*		
		if(no==1)
			dtempY2[i] =  RepeatChVal_Sig3[i][1]/1000;
		

		if(no==2)
			dtempY2[i] =  RepeatChVal_Sig3[i][10];
		if(no==3)
		...
		if(no==6)
			dtempY2[i] =  RepeatChVal_Sig3[i][7]+RepeatChVal_Sig3[i][9];
		if(no==7)
			dtempY2[i] =  RepeatChVal_Sig3[i][8]+RepeatChVal_Sig3[i][10];
*/			
...
//	SetAxisScalingMode (panel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 360);
		...
		if(no==0 || no == 2)
		{
			SetAxisScalingMode (mainpanel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 50.0);
//			SetAxisScalingMode (mainpanel, ctrlId, VAL_XAXIS , VAL_MANUAL, RepeatEncVal[0], RepeatEncVal[Index-1]);
			SetAxisScalingMode (mainpanel, ctrlId, VAL_LEFT_YAXIS , VAL_AUTOSCALE, 0, 0);
	
			SetAxisScalingMode (manualctrl, ctrlId2, VAL_XAXIS , VAL_MANUAL, 0, 50);
			SetAxisScalingMode (manualctrl, ctrlId2, VAL_LEFT_YAXIS , VAL_AUTOSCALE, 0, 0);
			
			
			SetAxisScalingMode (mainpanel, ctrlId, VAL_LEFT_YAXIS, VAL_MANUAL, YaxisLow,
								YaxisHigh);
			SetAxisScalingMode (manualctrl, ctrlId2, VAL_LEFT_YAXIS, VAL_MANUAL, YaxisLow,
								YaxisHigh);
		...
		if(no==0 || no == 2)
		{
			DeleteGraphPlot(mainpanel, ctrlId, -1, VAL_IMMEDIATE_DRAW);

			
			PlotXY(mainpanel, ctrlId, GngX, L_GraphData.NTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_WHITE);
			
			PlotXY(mainpanel, ctrlId, GngX, L_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			
			PlotXY(mainpanel, ctrlId, GngX, Lm_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			
			
			DeleteGraphPlot(manualctrl, ctrlId2, -1, VAL_IMMEDIATE_DRAW);


			PlotXY(manualctrl, ctrlId2, GngX, L_GraphData2.NTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_WHITE);
			PlotXY(manualctrl, ctrlId2, GngX, L_GraphData2.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
			
			PlotXY(manualctrl, ctrlId2, GngX, Lm_GraphData.PTol, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_GREEN);
...
//							VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1 , VAL_BLUE);
	if(no == 0 || no == 2)
		PlotXY(mainpanel, ctrlId, dtempX, dtempY2, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_MAGENTA);
	else
		PlotXY(mainpanel, ctrlId, dtempX, dtempY2, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_YELLOW);
		
	if(no == 0 || no == 2)
		PlotXY(manualctrl, ctrlId2, dtempX, dtempY2, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_MAGENTA);
	else
		PlotXY(manualctrl, ctrlId2, dtempX, dtempY2, Index, VAL_DOUBLE,VAL_DOUBLE,
										VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_YELLOW);
...
void	TurnDataGraphPlot2(int no,int	Index, int panel) // 1회전 데이터 그래프 출력
{
	...
	if( no == 2 || no == 3 )
	{
		...
		ctrlId2 = MANUALCTRL_GRAPH4;
	}
	else if( no == 1 || no == 0 )
	{
		ctrlId = MAINPANEL_GRAPH6;
		ctrlId2 = MANUALCTRL_GRAPH6;
	}
	...
	for(i = 0; i < Index; i ++) // data store
	{
		RepeatEncVal[i] = RepeatChVal_Sig3[i][3];//360.0*i/Index;
	}
...
//	SetAxisScalingMode (panel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 360);
		...
		if(no==0 || no == 2)
		{
			SetAxisScalingMode (mainpanel, ctrlId, VAL_XAXIS , VAL_MANUAL, 0, 50);
			SetAxisScalingMode (mainpanel, ctrlId, VAL_LEFT_YAXIS , VAL_AUTOSCALE, 0, 0);
...
//		dtempY2[i-Start_EncNo[0]] = RepeatEquResult[no][i-Start_EncNo[0]]; // calc data
		if(no==0)
			dtempY2[i] =  RepeatChVal_Sig3[i][6];
		if(no==1)
			dtempY2[i] =  RepeatChVal_Sig3[i][9];
		if(no==2)
			dtempY2[i] =  RepeatChVal_Sig3[i][10];
		if(no==3)
			dtempY2[i] =  RepeatChVal_Sig3[i][0]+RepeatChVal_Sig3[i][1];
...
//	if(Nick_Graph_Flag)
		...
		PlotXY(datapanel, DATAPANEL_GRAPH1, dtempX, dtempY2, 720, VAL_DOUBLE,VAL_DOUBLE,
										VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_MAGENTA);
...
void	GetGNgGrade(int	i, int	TestDataIndex)
{
	...
	{				
		...
		if(Mc[McID].Mode[0].FinalResult[i][TestDataIndex] < Retry_Data)
			Retry_Now = 1;
...
//	합격 불합격 판정, 측정값 화면표시, 색깔표시 화면 표시	
	for(i = 0; i < MeasPoints; i++)
	{
		...
		if((Model[i].Sig == GSt1Sig)|| (Model[i].Sig == GSt1Sig-1))
		{
			Retain_Data[i] = 0;
	...
	*/
				...
				data = Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1];
				
				if((data>= Model[i].NTol) && (data< Model[i].NCTol))
					Retain_Data[i] = 1;
				else if((data>= Model[i].NCTol) && (data< Model[i].PCTol))
					Retain_Data[i] = 2;
				else if((data>= Model[i].PCTol) && (data<= Model[i].PTol))
					Retain_Data[i] = 3;
...
/*	
		calc_out_data(pc121,0);
		calc_out_data(pc122,0);  
		calc_out_data(pc123,0);  
		calc_out_data(pc124,0);  
		calc_out_data(pc125,0);  
	if(!TotalGFlag)
		calc_out_data(pc120+Ng_No,1);														  
*/	
...
//	판정결과 PLC 전송용 PC Memory Clear
	...
	for(i = 0; i < MeasPoints; i++)
	{
		...
		{
			if(Model[i].Gap)
				calc_out_data(pc79+Model[i].Gap,1);
		}
							// 합격으로 초기화
	}
*/	

	for(i=pc160; i<=pc175; i++)
	{ 
		calc_out_data(i,0);
	}
	
	for(i=0; i<4; i++)  //MAX_RESULT_DATA2PLC=6
	{
			strcpy(Lan_Data_Char[i],"OK");
		
/*
		// 가공기 보정용 판정결과 전송 : 측정 결과값 BCD 변환후 PLC 전송용 메모리에 설정
		if( i < MeasPoints)
			BCDEncoder( Mc[McID].Mode[0].FinalResult[i][TestDataIndex]/1000, -1, 3, (pc64+ 16 * i), 1);///data1 pc64부터 시작		
		else
			BCDEncoder( 0.0 , -1, 3, (pc64 + 16 * i), 1);///data1 pc61부터 시작			
*/		
																				  
//		if (i >= MeasPoints)  goto KIN;
		if(Lan_Data[i]< IFTol[i][0])
//			||		// ㎛로 환산된 값이 전송범위 초과
//				(int)(Mc[McID].Mode[0].FinalResult[i][TestDataIndex])<-200 )			// ㎛로 환산된 값이 전송범위 초과  ///data400, data800 일 경우 판정3 출력
		{	// -NG
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[5][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[5][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[5][2]);
			
			strcpy(Lan_Data_Char[i],"-NG");
		}
		else if( Lan_Data[i]> IFTol[i][5] )			  //ModelID=>I
		{	// +NG
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[4][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[4][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[4][2]);
			strcpy(Lan_Data_Char[i],"+NG");
		}
		
		else if(Lan_Data[i]>= IFTol[i][0] &&
					Lan_Data[i]<= IFTol[i][1] )
		{	// --OK
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[3][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[3][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[3][2]);
			strcpy(Lan_Data_Char[i],"--OK");
		}
		
		else if(Lan_Data[i]>= IFTol[i][1] &&
					Lan_Data[i]<= IFTol[i][2] )
//		else if(Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] >= Model[i].NMTol &&
//					Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] <= Model[i].NCTol )
		{	// -OK
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[2][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[2][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[2][2]);
			strcpy(Lan_Data_Char[i],"-OK");
		}
		else if(Lan_Data[i]>= IFTol[i][3] &&
					Lan_Data[i]<= IFTol[i][4] )
//		else if( Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] >= Model[i].PCTol &&
//					Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] <= Model[i].PMTol )
		{	// +OK
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[1][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[1][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[1][2]);
			strcpy(Lan_Data_Char[i],"+OK");
		}
		else if(Lan_Data[i]>= IFTol[i][4] &&
					Lan_Data[i]<= IFTol[i][5] )
//		else if( Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] >= Model[i].PMTol &&
//					Mc[mcid].Mode[0].FinalResult[i][TestDataIndex] <= Model[i].PTol )
		{	// ++OK
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 0,IF_Data[0][0]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 1,IF_Data[0][1]);
			calc_out_data(pc160 + POINTS_PER_RESULT * i + 2,IF_Data[0][2]);
			strcpy(Lan_Data_Char[i],"++OK");
		}
...
KIN:
		if(i >= MeasPoints)												
		{	// +OK
			calc_out_data(pc77 + POINTS_PER_RESULT * i + 0,0);
			calc_out_data(pc77 + POINTS_PER_RESULT * i + 1,0);
			calc_out_data(pc77 + POINTS_PER_RESULT * i + 2,0);
		}
*/
	}
...
void	A_QRDataLabel(void)   //자동 QR 문자 생성
{
	...
	sprintf(Ex_Data[0],"[)>06");

	sprintf(data,"V%s", Model_Char[0]);
	strcat(Bar_Data, data);
	strcat(Ex_Data[0], data);
	
	sprintf(data,"%c",0x1D);	
	strcat(Bar_Data, data);
	
	
	sprintf(data,"P%s", Model_Char[1]);
	strcat(Bar_Data, data);
	strcat(Ex_Data[0], data);
...
/*
	...
	strcat(Bar_Data, data);
*/
	if(DayOrNightWork==0)//주간  //  년월일 공장 라인 주야 재료 
		sprintf(data,"T%s%s%s%s%sA2", YY_end1,Bar_Month1, Bar_Date1, Model_Char[2],Model_Char[3]);	
	else
		sprintf(data,"T%s%s%s%s%sB2", YY_end1,Bar_Month1, Bar_Date1, Model_Char[2],Model_Char[3]);	
		
	strcat(Bar_Data, data);
	strcat(Ex_Data[0], data);
	
	sprintf(data,"A%07d", Ok_cnt);	  //Mark_Cnt_No
	strcat(Bar_Data, data);
	strcat(Ex_Data[0], data);

	sprintf(data,"%c",0x1D);	  //Mark_Cnt_No
	strcat(Bar_Data, data);
	
	
	sprintf(data,"%08d", Ok_cnt);	  //Mark_Cnt_No
	SetCtrlVal(mainpanel, MAINPANEL_TEXT_DATA_1, data);
	
	
	
/*
	sprintf(data,"1A%s%c", Model_Char[4],0x1D);	//특이정보
	strcat(Bar_Data, data);

	sprintf(data,"M%s%c", Model_Char[5],0x1D);	//초도품구분
	strcat(Bar_Data, data);
	
	sprintf(data,"C%s%c", Model_Char[6],0x1D);	// 업체 영역
	strcat(Bar_Data, data);
*/	
...
void	A_QRDataLabel_send(void)   //자동 QR 문자 생성
{
	...
	Make_YYMMDD2();
	sprintf(Bar_Data,"[)>%%030A06%%029A");

	sprintf(data,"V%s%%029A", Model_Char[0]);
	strcat(Bar_Data, data);
	
	sprintf(data,"P%s%%029A", Model_Char[1]);
	strcat(Bar_Data, data);
	...
	sprintf(data,"A%07d%%029A", Ok_cnt);	  //Mark_Cnt_No
	strcat(Bar_Data, data);
...
/*
	...
	strcat(Bar_Data, data);
*/	
	sprintf(data,"%%030A%%004A");	
	strcat(Bar_Data, data);
	
	SetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_5, Bar_Data);
...
*/
	...
	sprintf(temp,"%s(ItemCode);",Model_Char[0]);
		strcat(DataFileBuff,temp);
	...
	sprintf(temp,"%s1%d(lot);",DateStamp, DayOrNightWork+1);
	strcat(DataFileBuff,temp);
...
//		strcpy(DataFileBuff,temp);	
    ...
    for( i = 0; i < MeasPoints; i++)										// 측정결과 저장
	{
		...
		sprintf(temp,"%4.3f", Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1]);
	   	strcat(DataFileBuff,temp);
...
void	TestFinish(void)
{
	...
	char	ctemp[256], ttemp[256], temp[100];
	char	ctemp1[100];
	double  i0;
...
/*	
		...
		TurnDataGraphPlot1(GSt1Sig-1,Graph_Index, mainpanel);   //shaft graph
		
		TurnDataGraphPlot2(GSt1Sig-1,Graph_Index, mainpanel);	//tube graph
...
//		SendResult2PLC(mcid, modeid, Mc[mcid].Mode[modeid].EdDataNum-1);  
		...
		if(GSt1Sig==2)
		{
			SetCtrlVal( mainpanel, MAINPANEL_GNG_2, ON);		
			L_TotalNo++;
			L_Ok++;
			...
			calc_out_data(pc19, 0); // NG
			calc_out_data(pc20, 0); // NG
			calc_out_data(pc21, 0); // NG
			
			
			if(	Retain_Data[1]> Retain_Data[0] )
				Retain_Data[0] = Retain_Data[1];
			 if(Retain_Data[0] == 3)
			 {
				calc_out_data(pc19, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_L,"A");
				
			 }
			 else if(Retain_Data[0] == 2)
			 {
				 calc_out_data(pc20, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_L,"B");
			 }
			 else if(Retain_Data[0] == 3)
			 {
				 calc_out_data(pc21, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_L,"C");
			 }
		...
		{
			SetCtrlVal( mainpanel, MAINPANEL_GNG_3, ON);		
			R_TotalNo++;
			R_Ok++;
			calc_out_data(pc80, 1); // OK
			calc_out_data(pc81, 0); // NG

			calc_out_data(pc82, 0); // NG
			calc_out_data(pc83, 0); // NG
			calc_out_data(pc84, 0); // NG
			calc_out_data(pc85, 0); // NG
			
			
			if(	Retain_Data[3]> Retain_Data[2] )
				Retain_Data[2] = Retain_Data[3];

			 if(Retain_Data[2] == 3)
			 {
				calc_out_data(pc82, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_R,"A");
				
			 }
			 else if(Retain_Data[2] == 2)
			 {
				 calc_out_data(pc83, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_R,"B");
			 }
			 else if(Retain_Data[2] == 3)
			 {
				 calc_out_data(pc84, 1); // NG
			    SetCtrlVal(mainpanel,MAINPANEL_RETAIN_R,"C");
			 }
...
//			calc_out_data(pc18, 1); // NG
		...
		if(!NGFlag_Retest && AutoRetry && Retry_Now)
		{
			NGFlag_Retest = TRUE; //재측정 On
			Mc[McID].Mode[ModeID].CycleIndex--;
			if(GSt1Sig==2)
			{
				SetCtrlAttribute(mainpanel, MAINPANEL_GNG_2, ATTR_OFF_TEXT, "재측정");
				SetCtrlVal( mainpanel, MAINPANEL_GNG_2, OFF);	
				
			
				calc_out_data(pc22, 1); // 재측정 요구 신호 on
			}
				
			if(GSt1Sig==4)
			{
				SetCtrlAttribute(mainpanel, MAINPANEL_GNG_3, ATTR_OFF_TEXT, "재측정");
				SetCtrlVal( mainpanel, MAINPANEL_GNG_3, OFF);	
				
			
				calc_out_data(pc85, 1); // 재측정 요구 신호 on
			}
...
//			SetCtrlVal(mainpanel, MAINPANEL_STATUSMSG, ctemp);
		...
		if(GSt1Sig==2)
		{
				SetCtrlAttribute(mainpanel, MAINPANEL_GNG_2, ATTR_OFF_TEXT, "불합격");
			SetCtrlVal( mainpanel, MAINPANEL_GNG_2, 0);		
			L_TotalNo++;
			L_Ng++;
		...
		{
				SetCtrlAttribute(mainpanel, MAINPANEL_GNG_3, ATTR_OFF_TEXT, "불합격");
			SetCtrlVal( mainpanel, MAINPANEL_GNG_3, 0);		
			R_TotalNo++;
			R_Ng++;
			calc_out_data(pc80, 0); // OK
			calc_out_data(pc81, 1); // NG
		}
...
///////////////
	if(TotalGFlag)
		sprintf(ttemp,"%d,%s,OK,",ModelID+1, CurrTime );
	else
		sprintf(ttemp,"%d,%s,NG,",ModelID+1, CurrTime );
		
	
   for( i = 0; i < MeasPoints; i++)										// 측정결과 저장
	{
			if(GFlag[i])
			    sprintf(temp, "%4.3f,OK,", Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1]);
			else
			    sprintf(temp, "%4.3f,NG,", Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1]);

		strcat(ttemp, temp);
	}

//	MessagePopup("test",ttemp);
  ...
  ///////////////	
	...
	GetCtrlVal(mainpanel, MAINPANEL_ZERO_NOW, &CurrAutoZeroCnt);
		
	CurrAutoZeroCnt++;
	
	SetCtrlVal(mainpanel, MAINPANEL_ZERO_NOW, CurrAutoZeroCnt);
	
	
	
	if( AutoZeroCntUseFlag && !ByPassFlag && !d_out[pc14] ) // 자동 영점조정
	{
...
void	CalculateSignal(int no)// low data에서 필요한 만큼만 뽑아서 RepeatChVal에 넣는다.
{
	int	i, j, k;
	double	LowData, HighData, data;
	...
	{
		if(no==1 || no==2)
		data = RepeatChValBase[i][2];
		else
		data = RepeatChValBase[i][3];
			
//		data = RepeatChValBase[i][no+1];
		
		if((data>= LowData) &&(data< HighData) )
		{
			for(k = 0; k < 4; k++)
			{
				RepeatChVal[j][k]=RepeatChValBase[i][k];
			}
			j++;
		}

			if((RepeatChValBase[i][2]>= HighData)&&  (GSt1Sig==1) )
				break;
			if((RepeatChValBase[i][2]<= LowData)&&  (GSt1Sig==2) )
				break;
			if((RepeatChValBase[i][3]>= HighData)&&  (GSt1Sig==3) )
				break;
			if((RepeatChValBase[i][3]<= LowData)&&  (GSt1Sig==4) )
				break;

//			if((RepeatChValBase[i][no+1]<= LowData)&&  ((GSt1Sig==2)||(GSt1Sig==4)))
//				break;
	...
	if(no == 1 ||no == 2)   //st qjsgh
		TurnDataGraphPlot1(GSt1Sig-1,Graph_Index, mainpanel);   //shaft graph
		
	if(no == 3 ||no == 4)   //st qjsgh
		TurnDataGraphPlot2(GSt1Sig-1,Graph_Index, mainpanel);	//tube graph
	...
	for(i=0;i< RepeatChIndex;i++)
	{
		for(k=0;k<2;k++)
		{
			if(	RepeatChVal[i][k] < 0.0)
				RepeatChVal[i][k]= (-1.0)*RepeatChVal[i][k];
...
//			GetUserEvent (0, &EventPanel, &EventCtrl);
			...
			{
//				CalculateSignal(Pgm[j].ST);// low data에서 필요한 만큼만 뽑아서 RepeatChVal에 넣는다.
				CalculateSignal(GSt1Sig);// low data에서 필요한 만큼만 뽑아서 RepeatChVal에 넣는다.
...
*/
				...
				//	최종 연산 결과 구하기
				EquResult[j] = EquResult[j] + Pgm[j].Comp1+ Pgm[j].NM2Tol;			//	연산 결과 보정(계산식 입력 Table의 보정값 대입)
				...
				if(GSt1Sig == 2)
				{
					if( Pgm[j].Disp == 1 && Pgm[j].Sig==2)			  
					{										  
						sprintf(temp," %f  ",EquResult[j]);	 
						strcat(ctemp,temp);					  
	
						EquResult3_Temp[j]=EquResult[j];			 
					}
				}				
				
				if(GSt1Sig == 3)
				{
					if( Pgm[j].Disp == 1 && Pgm[j].Sig==3)			  
					{										  
						sprintf(temp," %f  ",EquResult[j]);	 
						strcat(ctemp,temp);					  
	
						EquResult4_Temp[j]=EquResult[j];			 
					}
				}				
...
/*			
			if(GSt1Sig == 4)
			{
				...
				{														//추가
					if(Pgm[j].Disp == 1 && (Pgm[j].Sig==1 ||Pgm[j].Sig==2 || Pgm[j].Sig==3))				//추가
					{													//추가
...
/*
					if(Pgm[j].ST)							//	Display가 Yes인 Data만 최종 보관
					{
						Lan_Data[Pgm[j].ST-1] = EquResult[j];
						IFTol[Pgm[j].ST-1][0] = Pgm[j].NTol;
						IFTol[Pgm[j].ST-1][1] = Pgm[j].NMTol;
						IFTol[Pgm[j].ST-1][2] = Pgm[j].NCTol;
						IFTol[Pgm[j].ST-1][3] = Pgm[j].PCTol;
						IFTol[Pgm[j].ST-1][4] = Pgm[j].PMTol;
						IFTol[Pgm[j].ST-1][5] = Pgm[j].PTol;
...
*/
	...
	//		Cen = (*GraphData.PTol+*GraphData.NTol)/2;
			Cen = (GraphData.PTol[0]+GraphData.NTol[0])/2;
 ...
 //			SetAxisScalingMode (mainpanel, MAINPANEL_GRAPH1, VAL_LEFT_YAXIS, VAL_MANUAL, GraphData.NTol[SPCDisplaySig1]-1,GraphData.PTol[SPCDisplaySig1]+1);
	...
	//		SetCtrlVal(mainpanel, MAINPANEL_MEAS_POINT, Model[SPCDisplayPointNo].Name);
		...
		GraphData.HandleData = PlotY(mainpanel, MAINPANEL_GRAPH1, &GraphData.Data[SPCDisplaySig1], GraphData.DataNum, VAL_DOUBLE,
									VAL_CONNECTED_POINTS, VAL_DOTTED_SOLID_SQUARE, VAL_SOLID, 1, VAL_WHITE);
...
*/
 ...
 //			SetAxisScalingMode (mainpanel, MAINPANEL_GRAPH1, VAL_LEFT_YAXIS, VAL_MANUAL, GraphData.NTol[SPCDisplaySig2]-1,GraphData.PTol[SPCDisplaySig2]+1);
	...
	//		SetCtrlVal(mainpanel, MAINPANEL_MEAS_POINT, Model[SPCDisplayPointNo].Name);
		...
		GraphData2.HandleData = PlotY(mainpanel, MAINPANEL_GRAPH2, &GraphData2.Data[SPCDisplaySig2], GraphData2.DataNum, VAL_DOUBLE,
									VAL_CONNECTED_POINTS, VAL_DOTTED_SOLID_SQUARE, VAL_SOLID, 1, VAL_WHITE);
...
{
	int 	HfileAll,HfileOkNg, i,HfileLR;
	int		WorkerID;
	char	DataFileBuff[2048],DataFileBuff1[2048], TimeStamp[100], DataStamp[512], DataStamp1[512],  DateStamp[60],Temp[30];
	long	fileSize;
	char	fmt[10];
	char  bar1[30], bar2[30];
	...
	if(GSt1Sig==2)
		HfileLR = OpenFile(DataFileNameLFT,
								VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII);
	else
		HfileLR = OpenFile(DataFileNameRGT,
								VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII);
...
//								VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII);
	...
	sprintf(DataStamp, fmt, DateStamp);
	strcat(DataFileBuff, DataStamp);
	strcat(DataFileBuff1, DataStamp);
...
//	strcpy(DataFileBuff1, DataFileBuff);
			...
			GetCtrlVal(mainpanel, MAINPANEL_B2, bar1);
			GetCtrlVal(mainpanel, MAINPANEL_B1, bar2);
//		sprintf(DataStamp1,"%s - %s", bar1, bar2);
	
	sprintf(fmt, "%%%ds", DATAFILE_OPT_W);
	strcpy(TimeStamp,bar1);
	sprintf(DataStamp, fmt, TimeStamp);
	strcat(DataFileBuff, DataStamp);
	strcat(DataFileBuff1, DataStamp);

	sprintf(fmt, "%%%ds", DATAFILE_OPT_W);
	strcpy(TimeStamp,bar2);
	sprintf(DataStamp, fmt, TimeStamp);
	strcat(DataFileBuff, DataStamp);
	strcat(DataFileBuff1, DataStamp);
...
//	strcat(DataFileBuff1, DataStamp);
	...
	strncpy(DataFileBuff1, DataStamp, 8);
	WriteLine (HfileOkNg, DataFileBuff, -1);   

	WriteLine (HfileLR, DataFileBuff1, -1);   
...
//  Read 모델 수
	...
	if(ProbeIndex > 0)
	{
		...
		{
			ReadLine (ModelFileHandle, ModelFileBuff,-1);
			Scan(ModelFileBuff,"%s>%s%f", ProbeID[i],&ProbeV[i]);
...
//	int			 T_Point = 0; 
	...
	//(M)처음실행시 라벨색깔을 설정...
	SetCtrlAttribute (mainpanel, MAINPANEL_P1, ATTR_LABEL_BGCOLOR, VAL_GREEN);
...
//	GetCtrlVal(optpanel, OPT_SET_BYPASS3, &ByPassFlag3);
/////수정 1219
	GetCtrlVal(optpanel, OPT_SET_BYPASS4, &ByPassFlag4);
//	GetCtrlVal(optpanel, OPT_SET_BYPASS5, &ByPassFlag5);
//	GetCtrlVal(optpanel, OPT_SET_SPCDATANO1, &SpcDataNo);
...
*/					
			if(AutoZeroCntUseFlag && !Check_Master_Flag)
				calc_out_data(pc14, 1); 
...
void	SetGraphTol(void)
{
	...
	for(i = 0; i < MAX_PLOT_NUM; i++ )
	{
		...
		GraphData.NTol[i] = Model[SPCDisplaySig1].NTol;
		GraphData.PTol[i] = Model[SPCDisplaySig1].PTol;
		GraphData.NCTol[i] = Model[SPCDisplaySig1].NCTol;
		GraphData.PCTol[i] = Model[SPCDisplaySig1].PCTol;
		GraphData2.NTol[i] = Model[SPCDisplaySig2].NTol;
		GraphData2.PTol[i] = Model[SPCDisplaySig2].PTol;
		GraphData2.NCTol[i] = Model[SPCDisplaySig2].NCTol;
		GraphData2.PCTol[i] = Model[SPCDisplaySig2].PCTol;
/*		
		GraphData.NTol[i] = Model[SPCDisplayPointNo].NTol;
		GraphData.PTol[i] = Model[SPCDisplayPointNo].PTol;
		GraphData.NCTol[i] = Model[SPCDisplayPointNo].NMTol;
		GraphData.PCTol[i] = Model[SPCDisplayPointNo].PMTol;
		GraphData2.NTol[i] = Model[SPCDisplayPointNo].NTol;
		GraphData2.PTol[i] = Model[SPCDisplayPointNo].PTol;
		GraphData2.NCTol[i] = Model[SPCDisplayPointNo].NMTol;
		GraphData2.PCTol[i] = Model[SPCDisplayPointNo].PMTol;
*/
...
//// 1cycle data graph를 넣는다.		
		L_GraphData.NTol[i] = Model[0].NTol;
		L_GraphData.PTol[i] = Model[0].PTol;
		L_GraphData2.NTol[i] = Model[2].NTol;
		L_GraphData2.PTol[i] = Model[2].PTol;

		Lm_GraphData.PTol[i] = -Model[0].PTol;
		Lm_GraphData.PTol[i] = -Model[2].PTol;
...
//	SetCtrlVal( mainpanel, MAINPANEL_LNG, McSPC[0].NG+McSPC[1].NG+McSPC[2].NG );
	...
	SetCtrlVal( mainpanel, MAINPANEL_TOTALTESTNO_2, L_TotalNo );	// 전체 측정회수 표시
	SetCtrlVal( mainpanel, MAINPANEL_TOTALTESTNO_3, R_TotalNo );	// 전체 측정회수 표시
	...
	if((Model[SPCDisplayPointNo].Sig==1)||(Model[SPCDisplayPointNo].Sig==2))
...
//		LogDataFunc("TCP CONNECT", 1); 
		...
		sprintf(TCPSendBuff,"61,");
///yymmdd		
 		Make_YYMMDD1();
			strcat(TCPSendBuff,YMDChar);
///tube barcode
			GetCtrlVal(mainpanel, MAINPANEL_B2, temp);
			strcat(TCPSendBuff,temp);
		sprintf(temp,",");
			strcat(TCPSendBuff,temp);

//shaft barcode		
			GetCtrlVal(mainpanel, MAINPANEL_B1, temp);
			strcat(TCPSendBuff,temp);
		sprintf(temp,",");
			strcat(TCPSendBuff,temp);
		...
		if(TotalGFlag)
			...
			strcat(TCPSendBuff, "MAX,");
			
//station && grade			
			if(GSt1Sig==2)
			{
				strcat(TCPSendBuff, "1,");
				sprintf(DataStamp,"%d,", Retain_Data[0]);//상한
				strcat(TCPSendBuff, DataStamp);
			}
			else
			{
				strcat(TCPSendBuff, "2,");
				sprintf(DataStamp,"%d,", Retain_Data[2]);//상한
				strcat(TCPSendBuff, DataStamp);
					
			}

			if(Auto_1cycleFlag)
				strcat(TCPSendBuff, "CYCLE,");
			else
				strcat(TCPSendBuff, "AUTO,");
			

////shaft

		i =GSt1Sig-2;	
		sprintf(DataStamp,"%4.2f,", Model[i].NTol);//상한
		strcat(TCPSendBuff, DataStamp);

		sprintf(DataStamp,"%4.2f,", Model[i].PTol);//하한
		strcat(TCPSendBuff, DataStamp);
		
		sprintf(DataStamp,"%4.2f,", Mc[McID].Mode[0].FinalResult[i][Mc[McID].Mode[0].EdDataNum - 1]);
		strcat(TCPSendBuff, DataStamp);
		
		if(GFlag[i]==0) 
			sprintf(DataStamp,"NG,");
		else
			sprintf(DataStamp,"OK,");
		strcat(TCPSendBuff, DataStamp);
...
int		TestData2TCP(int mcid, int modeid, char* data)
{
	...
	char		TCPSendBuff[300], Temp[300];
			...
			sprintf(TCPSendBuff,"%s%c",data, 0x0D);  	
...
// 측정값 전송 via TCP/IP		
int		SendData2TCP2(int mcid, int modeid, int TestDataIndex)
{
	...
	if(ConnectToTCPServer (&TCPHandle, PortNo, ServerIP_2, ClientTCPCB, NULL, 2000) < 0)
	{	// 연결실패								
		//	상태 Flag 설정
		TCPErrFlag=TRUE;
    ...
    else	//	연결성공
   	{
    	...
    	switch (modeid)
        	{
        	case 0:				//	자동 측정모드
				Fmt(TCPSendBuff,"%s<%s", "$D");
	            break;
   
        	case 2:				//	R&R 측정모드
				Fmt(TCPSendBuff,"%s<%s", "$R");
	            break;

        	case 3:				//	Rework 측정모드
				Fmt(TCPSendBuff,"%s<%s", "$W");
	            break;

        	case 4:				//	주기 데이타 전송모드
				Fmt(TCPSendBuff,"%s<%s", "$DATA SEND COMMAND");
	            break;

        	case 5:				//	고장 데이타 전송모드
				Fmt(TCPSendBuff,"%s<%s", "$E");
	            break;
...
//			strcat(TCPSendBuff, Temp);		
		...
		for( i=0; i<6; i++)
		{
			if(Lan_Data[i] == 0.0)
				...
				Fmt(Temp,"%s<%f[w10p3]", Lan_Data[i]);
			strcat(TCPSendBuff, Temp);
		}

//		//	생산제품(ITEM) 쓰기
//		strcat(TCPSendBuff, "ItemNumber");
...
/*
		//	생산차종(ITEM) 쓰기
		Temp[0] = 0; 		
		sprintf(Temp,"%10s",ModelName[ModelID]);
		strcat(TCPSendBuff, Temp);
*/
IDLE:		
		//	종료표시 쓰기
    ...
    switch (event)
        ...
        case TCP_DATAREADY:
           if ((dataSize = ClientTCPRead (TCPHandle, receiveBuf,
                ...
                < 0)
                {
                SetCtrlVal (mainpanel, MAINPANEL_STATUSMSG, "Receive Error");
                }
            else
                {

					receiveBuf[dataSize] = '\0';
	                SetCtrlVal (mainpanel, MAINPANEL_STATUSMSG, receiveBuf);
					LogDataFunc(receiveBuf, 1); 
				
					sprintf(temp0,"%c%c",receiveBuf[0],receiveBuf[1]);
					sprintf(temp1,"C2");//마킹 전송 결과
					sprintf(temp2,"CO");//SCAN 결과
					
					
					if(!strcmp( temp0, temp1))
					{

						sprintf(temp0,"%c",receiveBuf[3]);
	...
	if((Model[SPCDisplayPointNo].Sig==1) ||(Model[SPCDisplayPointNo].Sig==2) )
	{
		...
		else
		{
			...
			for(j=0;j<MAX_PLOT_NUM ;j++)
			{
				...
				i = PlotY(mainpanel, MAINPANEL_GRAPH1, Center_Point, MAX_PLOT_NUM, VAL_DOUBLE,
											VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_CYAN);
 ...
 // 			SetAxisScalingMode (mainpanel, MAINPANEL_GRAPH1, VAL_LEFT_YAXIS, VAL_MANUAL, GraphData.NTol[SPCDisplaySig1]-1,GraphData.PTol[SPCDisplaySig1]+1);
	...
	else
	{
		...
		else
		{
				...
				GraphData2.HandleNTol = PlotY(mainpanel, MAINPANEL_GRAPH2, GraphData2.NTol, MAX_PLOT_NUM, VAL_DOUBLE,
											VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);

				GraphData2.HandlePTol = PlotY(mainpanel, MAINPANEL_GRAPH2, GraphData2.PTol, MAX_PLOT_NUM, VAL_DOUBLE,
											VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_RED);
										
										
				GraphData2.HandleNCTol = PlotY(mainpanel, MAINPANEL_GRAPH2, GraphData2.NCTol, MAX_PLOT_NUM, VAL_DOUBLE,
											VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_MAGENTA);
										
				GraphData2.HandlePCTol = PlotY(mainpanel, MAINPANEL_GRAPH2, GraphData2.PCTol, MAX_PLOT_NUM, VAL_DOUBLE,
											VAL_FAT_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_MAGENTA);										
  ...
  //			SetAxisScalingMode (mainpanel, MAINPANEL_GRAPH2, VAL_LEFT_YAXIS, VAL_MANUAL, GraphData2.NTol[SPCDisplaySig2]-1,GraphData2.PTol[SPCDisplaySig2]+1);
...
//			MessagePopup("pos", temp);
			if( control != MAINPANEL_PICTURE )
			{
				...
				for( i = 0; i < MeasPoints; i++ )
				{
					...
					{																		 
						...
						//선택한넘의 색깔을 바꿔BOA요~~
						SetCtrlAttribute (mainpanel, PointCtrl[i], ATTR_LABEL_BGCOLOR, VAL_GRAY); 
						SPCDisplayPointNo = i;
						break;
					}
				}
			...
			else // Graph and SPC disp change
			{
				if( control != MAINPANEL_PICTURE )
				{
					...
					if((Model[SPCDisplayPointNo].Sig == 1)|| (Model[SPCDisplayPointNo].Sig == 2))
						SPCDisplaySig1 =  SPCDisplayPointNo;
					else
						SPCDisplaySig2 =  SPCDisplayPointNo;
...
//				SetCtrlVal(mainpanel, MAINPANEL_INWORK_COUNT, Inworkcount);
			...
			GetCtrlIndex(mainpanel, MAINPANEL_RING,&i) ;
			GSt1Sig = i+1;
  		 	IdelStateTCP = 1; 
...
//				calc_out_data(pc12, 0); // 재측정
			...
			St1ReadIng = 1;
			MeasStart(1,GSt1Sig); // Start: ON,  Sig: 1
				break;
			...
			MeasStart(0, GSt1Sig); // Start: OFF,  Sig: 1
			St1ReadIng = 0;
...
//					GSt1Sig = 1;
					...
					for( RepeatChIndex = 0; RepeatChIndex < 1000; RepeatChIndex++ )
					{
						for(i = 0; i < MAX_CH; i++)
						{
							...
							if(i==2 )
							RepeatChVal[RepeatChIndex][i] = ((double)RepeatChIndex/10.0);//
							else if(i==3 )
							RepeatChVal[RepeatChIndex][i] = ((double)RepeatChIndex/10.0);//
							else if(i==0 )  //shaft 값
							RepeatChVal[RepeatChIndex][i] = ((double)RepeatChIndex/100 *2.0);
							else if(i==1 )  //shaft 값
							RepeatChVal[RepeatChIndex][i] = ((double)RepeatChIndex/100 *1.5);
			...
			case	_C_A:  // Left
					...
					for( RepeatChIndex = 0; RepeatChIndex < 1000; RepeatChIndex++ )
					{
						for(i = 0; i < MAX_CH; i++)
						{
							...
							// (0 ~ RAND_MAX == 32767) <<-- rand()
							if(i==2 )
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/10.0);// shaft 각도
							else if(i==3 )
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/10.0);// tube 각도
							else if(i==0 )  //shaft 값
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/100 *0.7);
							else if(i==1 )  //shaft 값
							RepeatChVal[RepeatChIndex][i] = ((double)(1000-RepeatChIndex)/100 *1.0);
						}
					}
...
//				JSenValSave(); // 확인용, test
			...
			ReadMeasData();
				...
				SetCtrlAttribute( mainpanel, MAINPANEL_BAR_DATA_5, ATTR_VISIBLE, TRUE);
				SetCtrlAttribute( mainpanel, MAINPANEL_BAR_DATA_6, ATTR_VISIBLE, TRUE);
				SetCtrlAttribute( mainpanel, MAINPANEL_BAR_DATA_7, ATTR_VISIBLE, TRUE);
...
//실제 전송 문자
         ...
         GetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_7, temp_barcode);
			calc_out_data(pc120, 0); // 마킹기 전송 ok
			calc_out_data(pc119, 0); // 마킹기 전송 ng

			
			   sprintf(mark_data,"C2,000%d,001,%s",ModelID+1,temp_barcode);
			   TestData2TCP(0,0, mark_data);
			
			   
			   
          GetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_3, temp_barcode);
		  strcpy(QR_A_Data, temp_barcode);
...
int CVICALLBACK A_Table2 (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	Point		cell;
	int			top, left, leftbtn, rightbtn, key;
...
int CVICALLBACK List_Status (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
int CVICALLBACK ManualCtrl_2 (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
//			SetCtrlAttribute (manualctrl, MANUALCTRL_M_VIEW_2, ATTR_TOP, 3);
//			SetCtrlAttribute (manualctrl, MANUALCTRL_MANUALTEST_M, ATTR_TOP, 183);
//			SetCtrlAttribute (manualctrl, MANUALCTRL_MANUALTEST_M_2, ATTR_TOP, 415);
//			SetCtrlAttribute (manualctrl, MANUALCTRL_M_GNG_3, ATTR_TOP, 299);
//			SetCtrlAttribute (manualctrl, MANUALCTRL_M_GNG_4, ATTR_TOP, 531);

					
//			SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_VISIBLE, FALSE);
...
void  Connect_Tcp(void)
{
	...
	{	// 연결실패								
		...
		TCPErrFlag=TRUE;
		LogDataFunc("TCP FAIL 1", 1);
		calc_out_data(pc116, 1); // 마킹기 lan alarm
	}
    else
	{
		
//		SendData2ATLAS(0, 0, 1); //atlas comm start
		LogDataFunc("MARK TCP Connect OK", 1);
		SetCtrlVal(mainpanel, MAINPANEL_LED_MC_4,1);
		
		SetCtrlAttribute(lanpanel, LAN_SET_CONNECT_MARK, ATTR_DIMMED ,TRUE);
...
int CVICALLBACK TotalTestNo3 (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
int CVICALLBACK B1_Select (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
int CVICALLBACK B2_Select (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
