...
//  Msg Panel
extern		int			lanpanel, commpanel, pospanel,markpanel,err1panel,optpanel,interdata, workref, menu_item, datapanel;
...
extern	int		AmpPortNo, PlcPortNo, MarkPortNo, AUX2PortNo;
...
typedef	struct
		{
			double	NTol[MAX_ACQ_CH_PER_CYCLE], PTol[MAX_ACQ_CH_PER_CYCLE];
		} GRAPHDATA2;

//extern	GRAPHDATA2	GraphData2;
...
typedef		struct {
	...
	double		Comp, NTol, PTol, Comp1, NCTol, PCTol,NMTol,PMTol,NM2Tol,PM2Tol ;
} PGM;
...
//	char 	Date[MAXMEASNUM][20];           // 측정 날짜KindOfTol
	...
	int	   	measNum;  
	char 	GNG1[MAXMEASNUM][50];           // marking 문자
	char 	GNG2[MAXMEASNUM][50];           // marking 문자
	char 	GNG3[MAXMEASNUM][20];           // marking 문자
//측정회수
};
...
extern	double		RepeatChValBase[MAX_ACQ_CH_PER_CYCLE][MAX_CH];
...
extern	char		DataFileNameOk[], DataFileNameNg[], DataFileNameAll[], DataFileNameLFT[], DataFileNameRGT[];//, DataFile[256];
...
extern	int			ByPassFlag, ByPassFlag2, ByPassFlag3, ByPassFlag4, ByPassFlag5;
extern	int			PlcPcOff;
...
extern	int			AutoZeroChkHour, AutoZeroHourUseFlag, CurrAutoZeroHour; // 영점조정 시간간격 설정변수
...
extern	int     Max_Comp;
extern	int		AutoRetry, ReverseFlag, AMeasMode;
...
GRAPHDATA2	L_GraphData, L_GraphData2, Lm_GraphData, Lm_GraphData2;
...
extern		unsigned  long	Servo_Position[];// 1: 이송부 ready, 2:이송부 전진, 3:마킹부 전진, 4: 측정부 전진
extern	unsigned  long	Shaft_Angle;
extern	unsigned  long	Tube_Angle;

extern		double Torque_Data;

extern	double	Work_Dia, Roller_Dia, Nick_Comp,Nick_Comp1[],Enc_Ratio;
extern	int			EncPlsNum, EncDir,EncChannel;
...
extern	int			Inworkcount;
 extern	int		Bar_Send_Flag, A_Bar_Send_Flag;
...
extern	int		Total_cnt, Ng_cnt, Ok_cnt, Mark_Cnt_No;
...
extern	double  S_fft[][100];		// order값
...
extern	double	Mark_X[],Mark_Y[],Mark_Z[];
extern	char	Mark_Text[][50];
...
extern	char	Mark_Data[][4][30];
extern	char	Mark_Data_Shift[][4][30];
...
extern	int		Zero_Master_Flag,Check_Master_Flag;// 제로 감도 측정 유무 
extern	int		M_Gng;// 수동 측정 결과
extern		char  temp_barcode[];
extern	char	QR_A_Data[];
...
extern	int		LowLogFlag;
extern	char	Ex_Data[][100];
void  Connect_Tcp(void) ;
 void	SaveMarkData2(void);
void	ReadMarkData2(void);
extern	char	Lan_Data_Char[][10];
extern	double	Shaft_Low, Shaft_High, Tube_Low, Tube_High;
extern	int	RepeatChIndexBase;
void	StartAnalog(void);
extern	int	L_MeasPoints, R_MeasPoints;
extern	int		L_TotalNo, R_TotalNo, L_Ok,R_Ok, L_Ng, R_Ng;
void		GraphPlot_SPC(void);
extern	int	MeasCnt;
void	Save_CsvFile(void) ;
 
extern	double	Retry_Data;
 void	ReadSenval1(void) ;
 void	ReadSenval2(void) ;
 void	ReadSenval3(void) ;
extern	int	Auto_1cycleFlag;
extern	int		Bar_Select;
extern	double	YaxisLow, YaxisHigh;
