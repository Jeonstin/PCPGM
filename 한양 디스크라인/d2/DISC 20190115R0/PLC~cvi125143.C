...
int		Bar_Send_Flag=0,Bar_Send_Flag2=0 ,A_Bar_Send_Flag = 0;
...
//		d_in[j+320] =d_in[j+320+160+160]; 
		d_in[j+320] =d_in[j+320+160+160+160+160+160]; 
...
//		AmpStart(0); // amp read cancel.
	...
	ManualCtrlSt1Dimmed(0);
			if(language==0)
			{
				SetCtrlAttribute(manualctrl, MANUALCTRL_MANUALTEST_1, ATTR_LABEL_TEXT, "FRF 측정시작");
			}
			else if(language==1)
...
//마킹 회전 수		
		...
		{
			if( data & bPos )
				calc_out_data(pc128 + i, SET);
			else
				calc_out_data(pc128 + i, RESET);
			bPos <<= 1;
		}
...
//SCALE CYL ACT NO		
 ...
 /*
		...
		{
			if( data & bPos )
				calc_out_data(pc592 + i, SET);
			else
				calc_out_data(pc592 + i, RESET);
			bPos <<= 1;
		}
*/
...
void  Sgs_Send(void)
{
...

	{
		...
		{
			if( data & bPos )
				calc_out_data(pc512 + i, SET);
			else
				calc_out_data(pc512 + i, RESET);
			bPos <<= 1;
		}
...

	{
		...
		{
			if( data & bPos )
				calc_out_data(pc544 + i, SET);
			else
				calc_out_data(pc544 + i, RESET);
			bPos <<= 1;
		}
...
//			LogDataFunc("RESET 신호", 0);
	...
	if( (d_in[plc8] != def_in[plc8]) ) 
	{
		if( d_in[plc8] )
		{
			...
			calc_out_data(pc89, 0);
			calc_out_data(pc93, 0);
...
//			SetCtrlVal(tabpanel, TABPANEL_LED_DATA_7,FALSE);
	...
	if( (d_in[plc29] != def_in[plc29]) ) 
	{
		if( d_in[plc29] )
		{
			...
			SetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_6, " ");
...
//				SetCtrlVal(mainpanel, MAINPANEL_LED_DATA_9,TRUE);
//				SetCtrlVal(tabpanel, TABPANEL_LED_DATA_9,TRUE);
		}
		else
		{
 ...
 void CT_Read(void)
{
	...
	ct = (double)CTPos/10.0;
	
		SetCtrlVal(mainpanel, MAINPANEL_MARK_CNT_NO_2,ct);
}
