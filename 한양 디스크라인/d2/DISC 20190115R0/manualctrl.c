		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	{
		case EVENT_LEFT_CLICK: 
			calc_out_data(pc96, 1); // servo move
			break;
		case EVENT_COMMIT:
		case EVENT_LOST_FOCUS:
...
/*		
	...
	for( i = 0; i < 16; i++ )
	{
		...
		SetCtrlVal(mainpanel, MAINPANEL_MARK_CNT_NO_2,MARK_CNT_NO_2);	
/*
	bPos = 1;
	Servo6CurPos = 0;
	for( i = 0; i < 32; i++ )
		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		...
		//	SetManuCtrlTableData2();
			...
			calc_out_data(pc142, 1); // 수동 측정요구 ST1
