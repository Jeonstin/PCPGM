...
void	GetGNgGrade(int	i, int	TestDataIndex)
{
	...
	if((Mc[McID].Mode[0].FinalResult[i][TestDataIndex] < Model[i].NCTol) && Model[i].Gap==1) 
	{
		GFlag[i] = FALSE;	// 불합격
		TotalGFlag = FALSE;	// 모든 측정 Point에 대하여 한 Point라도 불량이면 Reset
		return;
	}
	else if((Mc[McID].Mode[0].FinalResult[i][TestDataIndex] < Model[i].NTol) && Model[i].Gap==0) 
	{
		GFlag[i] = FALSE;	// 불합격
		TotalGFlag = FALSE;	// 모든 측정 Point에 대하여 한 Point라도 불량이면 Reset
		return;
