		...
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
		...
		//	DisplayPanel(zerosetup); // 오토 마스터링 수동화면 전환 안될시
...
///////////////////
					...
					GetCtrlVal(interdata, INTPANEL_BYPASS2, &ByPassFlag2);//NICK BYPASSS
					GetCtrlVal(interdata, INTPANEL_BYPASS4, &ByPassFlag4);//NICK BYPASSS
					GetCtrlVal(interdata, INTPANEL_BYPASS6, &ByPassFlag);//NICK BYPASSS
...
//180604 NG 종류 추가	
	...
	calc_out_data(pc126,0);
//	calc_out_data(pc127,0);
...
void	SendResult2PLC(int McID, int ModeID, int TestDataIndex)
{
	...
	{
		if(j==2)
		data = (int)(Mc[McID].Mode[0].FinalResult[j][TestDataIndex]*1000);
		else
		data = (int)(Mc[McID].Mode[0].FinalResult[j][TestDataIndex]*10);
		...
		{
			if( data & bPos )
				calc_out_data(pc480+j*16 + i, SET);
			else
				calc_out_data(pc480+j*16 + i, RESET);
			bPos <<= 1;
		}
		...
		{
			if( data & bPos )
				calc_out_data(pc560 + i, SET);
			else
				calc_out_data(pc560 + i, RESET);
			bPos <<= 1;
		}
		...
		{
			if( data & bPos )
				calc_out_data(pc576 + i, SET);
			else
				calc_out_data(pc576 + i, RESET);
			bPos <<= 1;
		}
...
//	int		mcid = 0, modeid=0;
//	long	fileSize;
	char		temp[100],Temp[100];
	char		DateStamp[512],TimeStamp[512],temp1[100];
//	int			meas_counter=0;
//	char	lineBuff[128];
...
//	sprintf(temp,"%s:",DateStamp);
	...
	strcpy(WorkingDirectory, "C:\\GunSolution");
//	if( SetDir(WorkingDirectory) )			//	Directory가 존재하지 않으면
//		MakeDir(WorkingDirectory);				//	Directory생성
...
// Project Directory로 환원
	...
	sprintf(DataStamp, "%s.txt",DateStamp);  // 10-character string Date in MM-DD-YYYY format 
//	sprintf(DataStamp, "amsys.txt");
...
*/
	DataFileHandle = OpenFile (WorkingDirectory, VAL_WRITE_ONLY, VAL_APPEND, VAL_ASCII);
		...
		sprintf(temp,"%s(LINE);",WorkerName[WorkerID]);
...
//		strcpy(DataFileBuff,temp);	
			...
			if(TotalGFlag==1)
	{
		...
		strcat(DataFileBuff,temp);
		strcat(DataFileBuff,"(MARK);");
		
	}
	else
	{
		sprintf(temp,"NG(OK?NG);",DateStamp, DayOrNightWork+1);
		strcat(DataFileBuff,temp);
		strcat(DataFileBuff,"NO MARK(MARK);");
	}
		...
		if( Frf_Flag == FALSE )
		{
			sprintf(temp,"NG(FRF OK?NG);");
		}
		else								//	측정값이 OK구간이면
		{
			sprintf(temp,"OK(FRF OK?NG);");
		}
		strcat(DataFileBuff,temp);   
		
		    for( i = 7; i < MeasPoints; i++)										// 측정결과 저장
			{
...
//			A_QRDataLabel(); //하부
			...
			A_MarkingDataLabel(); //하부
	   				
//		  A_MarkingDataLabel2(); //상부, 중간	
...
// 수동 LR 신호 보냄
			calc_out_data(pc104, 0); //
			calc_out_data(pc105, 0); //

			GetCtrlVal(manualctrl, MANUALCTRL_MODEL_LR, &i);
			if(i)
				calc_out_data(pc105, 1); //
			else
				calc_out_data(pc104, 1); //
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_LEFT_CLICK:
			...
			//SPCDisplayPointNo = cell.y-1;
			//SPCDisplayPointNo = cell.y+9;
			SPCDisplayPointNo = cell.y+6;
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal(manualctrl, MANUALCTRL_MODEL_LR, &no);
			if(no)
				calc_out_data(pc105, 1); //
			else
				calc_out_data(pc104, 1); //
