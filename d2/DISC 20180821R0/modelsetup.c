...
//							MakeRect (1, MODEL_NAME_COL, MAX_POINT, 1), "");
	...
	SetTableCellRangeVals(modelsetup, MODELSETUP_DATA,
		...
		for( i = 0; i < MeasPoints; i++ )
		{
			...
			SetTableCellRangeAttribute (modelsetup, MODELSETUP_DATA, MakeRect (1, 1, MeasPoints, 6), ATTR_TEXT_POINT_SIZE, 24);			

	
	for( i = 0; i < MeasPoints; i++ )
	{
			SetTableCellRangeAttribute (modelsetup, MODELSETUP_DATA, MakeRect(i + 1, MODEL_COMP_COL, 1, 5), ATTR_PRECISION, Model[i].Unit);
	}
