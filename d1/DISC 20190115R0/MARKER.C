...
int		MarkFont[3], MarkHeight[3], MarkPitch[3], MarkPosX[3], MarkPosY[3],MarkPosZ[3], MarkDir[3], MarkWidthLimit[3];
...
// Project Directory로 환원
	...
	if( GetFileInfo (WorkingDirectory, &fileSize) ) // file이 존재하면
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			ReadFile(h_file, (char *)&MarkPosZ[i], sizeof(int));
		...
		SetCtrlVal(markpanel, MARK_SET_MARKPOSZ, MarkPosZ[0]);
		SetCtrlVal(markpanel, MARK_SET_MARKWDLIMIT, MarkWidthLimit[0]);
		
		
    	MarkPosX[1] = MarkPosY[1] = MarkPosZ[1] = 10; 
    	MarkPosX[2] = MarkPosY[2] = MarkPosZ[2] = 10;
    ...
    {
    	MarkPosX[0] = MarkPosY[0] = MarkPosZ[0] =10;
    	MarkPosX[1] = MarkPosY[1] = MarkPosZ[1]=10; 
    	MarkPosX[2] = MarkPosY[2] = MarkPosZ[2]=10; 
...
// Project Directory로 환원
	...
	for(i = 0; i < 3; i++)
	{
		...
		WriteFile(h_file, (char *)&MarkPosZ[i], sizeof(int));
...
void  RereadMarkData(void)
{
	...
	GetCtrlVal(markpanel, MARK_SET_MARKPOSZ, &MarkPosZ[0]);
...
//		sprintf(YY_end1,"%c", ((iTemp) + 'A')); // 년: 2020:A, 2021:B, ~
		...
		iTemp = atoi(temp) ;
		if(iTemp==1)
			sprintf(Bar_Month1,"%s", "JA"); 
		if(iTemp==2)
			sprintf(Bar_Month1,"%s", "FE"); 
		if(iTemp==3)
			sprintf(Bar_Month1,"%s", "MR"); 
		if(iTemp==4)
			sprintf(Bar_Month1,"%s", "AP"); 
		if(iTemp==5)
			sprintf(Bar_Month1,"%s", "MY"); 
		if(iTemp==6)
			sprintf(Bar_Month1,"%s", "JN"); 
		if(iTemp==7)
			sprintf(Bar_Month1,"%s", "JL"); 
		if(iTemp==8)
			sprintf(Bar_Month1,"%s", "AU"); 
		if(iTemp==9)
			sprintf(Bar_Month1,"%s", "SE"); 
		if(iTemp==10)
			sprintf(Bar_Month1,"%s", "OC"); 
		if(iTemp==11)
			sprintf(Bar_Month1,"%s", "NO"); 
		if(iTemp==12)
			sprintf(Bar_Month1,"%s", "DE"); 
...
/////////180523
				if(DayOrNightWork==0)//주간
					sprintf(data,"%s%s%0004d",YY_end1,Day_Char,Ok_cnt);
				else
					sprintf(data,"%s%s%0004d",YY_end1,Night_Char,Ok_cnt);
...
//					sprintf(data,"%s%s%s%d",YY_end1,Bar_Month1, Bar_Date1,Line_Name);
			...
			{
				sprintf(data,"%0004d",Mark_Cnt_No);	
			}
			...
			{
				if(DayOrNightWork==0)//주간
					sprintf(data,"%s%s%s1 %0004d",YY_end1,Bar_Month1, Bar_Date1,Mark_Cnt_No);
				else
					sprintf(data,"%s%s%s2 %0004d",YY_end1,Bar_Month1, Bar_Date1,Mark_Cnt_No);
					
			}
			else if(Mark_Type1[i+8] == 7)
			{
					sprintf(data,"%s%s%s%d",YY_end1,Bar_Month1, Bar_Date1,Line_Name);
...
/////
	...
	for(i=0;i<4;i++)
	{
			//각인 문자
			sprintf(data,"");
			if(Mark_Type1[i+12] == 0)
			{
				sprintf(data,"%%1@");	
			}
			else if(Mark_Type1[i+12] == 1)
			{
				sprintf(data,"%%2@");	
			}
			else if(Mark_Type1[i+12] == 2)
			{
//					sprintf(data,"%s%s%s%d",YY_end1,Bar_Month1, Bar_Date1,Line_Name);
					sprintf(data,"%s%s%s",YY_end1,Bar_Month1, Bar_Date1);
					
			}
			else if(Mark_Type1[i+12] == 3)
			{
				sprintf(data,"%0004d",Mark_Cnt_No);	
			}
			else if(Mark_Type1[i+12] == 4)
			{
				sprintf(data,"%s",Mark_Text[i+12]);	
			}
			else if(Mark_Type1[i+12] == 5)
			{
				if(DayOrNightWork==0)//주간
					sprintf(data,"1");
				else
	...
	SetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_11, Mark_Bar_Data);
...
//	calc_out_data(pc75 , 1);
	...
	SetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_2, Bar_Data);
////////
	sprintf(Bar_Data,"");
	for(i=0;i<4;i++)
	{
...
//	calc_out_data(pc75 , 1);
	...
	SetCtrlVal(mainpanel, MAINPANEL_BAR_DATA_10, Bar_Data);
